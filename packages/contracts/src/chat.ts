...
export type ChatStreamChunk =
  ...
  /**
   * #927: rev 单调递增版本标识 — 前端按 rev 幂等防乱序（rev ≤ 已应用值忽略）。
   ...
   * #1113: deck_version = edit_deck_bytes 写回后的 pptx 工件版本
   * （version === artifactId，单字段足够）— 已打开的 deck 画布据此实时
   * 拉取新字节；字节本身绝不走 SSE（体积大），前端按版本号去重后再 GET。
   */
  | { type: 'doc_updated'; body: string; summary?: string; title?: string; deck?: DeckWire | null; rev?: number; updatedAt?: string; projection?: BlockProjection; section_meta?: SectionMetaMap; changed_sections?: Array<{ id: string; heading: string }>; deck_version?: string }
