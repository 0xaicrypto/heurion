...
export function htmlToMarkdown(html: string): string {
  ...
  // #1130: 只在**两侧都是单词字符**（字母/数字含 CJK）时反转义 — 标点旁的
  // `\_` 是强调定界符风险位（`(_draft_)` 还原后会被解析成斜体,下划线丢失,
  // 静默改内容），必须保持转义。lookahead 只消费左字符,链式 `a\_b\_c` 不漏。
  return md.replace(/([\p{L}\p{N}])\\_(?=[\p{L}\p{N}])/gu, '$1_');
}
