...
export function MemoryGraphPage() {
  ...
  const loadData = useCallback(() => {
    ...
    setError(null);
    Promise.all([
      api.getFindings(hash).catch(() => [] as MemoryNode[]),
      api.getMedications(hash).catch(() => [] as MemoryNode[]),
      api.getMemoryTimeline(hash).catch(() => [] as TimelineEntry[]),
    ])
      .then(([f, m, t]) => {
        setFindings(f);
        setMedications(m);
        ...
        <>
          <section className="mb-8">
            <div className="mb-3 flex items-center gap-2">
              ...
              <Badge variant="default">{findings.length}</Badge>
            </div>
            {findings.length === 0 ? (
          ...
          <section className="mb-8">
            <div className="mb-3 flex items-center gap-2">
              ...
              <Badge variant="default">{medications.length}</Badge>
            </div>
