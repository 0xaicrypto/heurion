 ...
 *   - ppt/slides/_rels/slideN.xml.rels（chart / diagramDrawing 关系）
...
function chartPartXml(chart: FixtureChart): string {
  ...
  const serXml = chart.series
    .map((ser) => `<c:ser><c:idx val="0"/><c:order val="0"/><c:tx><c:strRef><c:f>Sheet1!$B$1</c:f><c:strCache><c:ptCount val="1"/><c:pt idx="0"><c:v>${ser.name}</c:v></c:pt></c:strCache></c:strRef></c:tx><c:cat><c:strRef><c:f>Sheet1!$A$2:$A$9</c:f><c:strCache><c:ptCount val="${chart.categories.length}"/>${chart.categories
      .map((cat, i) => `<c:pt idx="${i}"><c:v>${cat}</c:v></c:pt>`)
      .join('')}</c:strCache></c:strRef></c:cat><c:val><c:numRef><c:f>Sheet1!$B$2:$B$9</c:f><c:numCache><c:formatCode>General</c:formatCode><c:ptCount val="${ser.values.length}"/>${ser.values
...
function diagramDrawingXml(shapes: string[], positions?: Array<{ x: number; y: number }>): string {
  ...
  const spXml = shapes
    ...
    .join('')
  return `${XML_DECL}<dsp:drawing xmlns:dsp="${NS.dsp}" xmlns:a="${NS.a}"><dsp:spTree><dsp:nvGrpSpPr/><dsp:grpSpPr/>${spXml}</dsp:spTree></dsp:drawing>`
}
...
export function buildPptxFixture(slides: FixtureSlide[]): BuiltPptx {
  ...
  slides.forEach((slide, i) => {
    ...
    if (slide.smartArt) {
      ...
      if (!slide.smartArt.missingDrawing) {
        entries.push({ name: `ppt/diagrams/${drawingPart}.xml`, data: diagramDrawingXml(slide.smartArt.shapes, slide.smartArt.positions) })
      }
      rels.push(`<Relationship Id="${dmRid}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/diagramData" Target="../diagrams/data${diagramIdx}.xml"/>`)
      if (!slide.smartArt.missingDrawing) {
        rid += 1
        rels.push(`<Relationship Id="rId${rid}" Type="http://schemas.microsoft.com/office/2007/relationships/diagramDrawing" Target="../diagrams/${drawingPart}.xml"/>`)
      }
      frames.push(smartArtFrame(dmRid, `rId${rid}`, `rId${rid}`, `rId${rid}`))
    }
    ...
    if (slide.notes) {
      entries.push({ name: `ppt/notesSlides/notesSlide${n}.xml`, data: notesSlideXml(slide.notes) })
    }
  })
