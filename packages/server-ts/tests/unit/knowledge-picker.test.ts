...
function nodes(): PickerNode[] {
  return [
    { stableId: 'article_atr', type: 'article', title: 'ATR 通路综述', content: '放疗抵抗机制与 DNA 损伤修复。', updatedAt: 1 },
    { stableId: 'doc_protocol', type: 'document', title: 'NSCLC001 试验方案', updatedAt: 2 },
    { stableId: 'article_tki', type: 'article', title: '三代 EGFR-TKI 研究', content: 'PFS 显著延长。', updatedAt: 3 },
  ]
}
...
describe('#633 knowledge picker unified search', () => {
  test('词法路: 标题子串命中,空查询返回全量', async () => {
    ...
    expect(hits.map((h) => h.node.stableId)).toContain('article_atr')
    expect(hits.map((h) => h.node.stableId)).not.toContain('article_tki')
  ...
  test('向量路: 语义相近词命中(搜"放疗抵抗"命中 ATR 论文)', async () => {
    const embedding = stubEmbedding([
      { stableId: 'article_atr', content: '放疗抵抗机制…', type: 'article', score: 0.7 },
    ])
    const hits = await searchPickerItems(nodes(), '放疗抵抗', embedding)
    expect(hits.some((h) => h.node.stableId === 'article_atr')).toBe(true)
    // 词法无命中时向量路仍能召回
