...
import { normalizeSynthesizedSummary } from '../../src/memory/summary-contract.js'
import { summarySynthesisPrompt, SUMMARY_SYNTHESIS_PERSONA } from '../../src/memory/prompts.js'
...
describe('#813 normalizeSynthesizedSummary — structured path', () => {
  test('builds markdown with title first line + 结论/依据/注意事项 sections', () => {
    const out = normalizeSynthesizedSummary(STRUCTURED, ALLOWED)!
  ...
  test('legacy {title, content} passes through with title-first invariant intact', () => {
    const out = normalizeSynthesizedSummary({ title: 'T', content: '正文内容' }, ALLOWED)!
  ...
  test('legacy content already starting with title is not double-prefixed', () => {
    const out = normalizeSynthesizedSummary({ title: 'T', content: 'T\n\n正文' }, ALLOWED)!
    expect(out.content).toBe('T\n\n正文')
  })

  test('missing title → null; missing conclusion without content → null', () => {
    expect(normalizeSynthesizedSummary({ conclusion: 'x' }, ALLOWED)).toBeNull()
    expect(normalizeSynthesizedSummary({ title: 'T' }, ALLOWED)).toBeNull()
  })

  test('non-object / null input → null', () => {
    expect(normalizeSynthesizedSummary(null, ALLOWED)).toBeNull()
    expect(normalizeSynthesizedSummary('str', ALLOWED)).toBeNull()
  })

  test('en lang renders English section headers and confidence labels', () => {
    const out = normalizeSynthesizedSummary(STRUCTURED, ALLOWED, { lang: 'en' })!
  ...
  test('caveats coerced from a single string', () => {
    const out = normalizeSynthesizedSummary({ ...STRUCTURED, caveats: '单一字符串 caveat' }, ALLOWED)!
    expect(out.content).toContain('- 单一字符串 caveat')
  })
})

describe('#813 summarySynthesisPrompt — contract instructions', () => {
  ...
  test('en persona prompt (regenerate path) demands the same contract in English', () => {
    const prompt = summarySynthesisPrompt('[fact_aaa] x', SUMMARY_SYNTHESIS_PERSONA.researcherEn)
