...
### 19.6 研究 ↔ 论文联动（科研闭环数据管道）

**现状断裂**：研究模块（ResearchStudy/Enrollment/Observation/Assessment）与论文模块（Doc）无关联——研究数据无法自动进入论文。

**目标**：研究 → 论文的数据管道，一个研究可产出多篇论文。

**数据模型扩展**：
```prisma
model Doc {
  ...
  studyId   String? @map("study_id")   // 新增：关联研究
}
model ResearchStudy {
  ...
  papers Doc[]   // 新增
...
| 联动 | 方向 | 说明 |
|---|---|---|
| 1. 创建论文关联研究 | 研究→论文 | 研究详情"写论文"→ 关联 Doc；或写作新建选研究 |
| 2. 方法自动生成 | 研究→论文 | 从 Screening 规则/统计方法/终点生成方法初稿 |
| 3. 结果注入统计 | 研究→论文 | 统计输出（#361）注入结果段 + 图表 |
| 4. 标题/摘要带背景 | 研究→论文 | 研究名称/目的 → 标题/摘要初稿 |
| 5. 投稿数据源 | 论文→投稿 | 选刊输入来自论文（#382） |

**用户流程**：研究（#8-14）→ 统计（#361）→ 论文（#383）→ 投稿（#382）——科研闭环完整化。

**与 #14 关系**：#14（论文草稿）是核心，本设计是它的数据管道 + 投稿延伸。
